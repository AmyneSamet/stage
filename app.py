...
from cv_parser import (get_mail,rank_cvs_exp,send_mail,recommend_jobs, read_data, job_by_source,rank_cvs,job_by_type,recommend_jobs_by_experience, extract_source,
                 job_type_by_source, most_offered_company, top_skills,most_frequent_jobs,recommend_by_skills_and_experience)
                ...
                with st.expander(f"Details for {row['CV Name']} ({row['Similarity']:.2f} Match)"):
                    ...
                    with col2:
                        if st.button(f"Send Email", key=f"send_email_{index}"):
                            ...
                            st.success(f"E-mail envoyé à {row['Email']}")
